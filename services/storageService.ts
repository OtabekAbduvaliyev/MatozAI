...
const DB_NAME = 'MatozAIDB';
const DB_VERSION = 2; // Bumped version to ensure clean schema
  ...
  async getSessions(): Promise<SavedSession[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        const results = (request.result as SavedSession[]).sort((a, b) => b.timestamp - a.timestamp);
        resolve(results);
      };

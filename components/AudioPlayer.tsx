...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ 
  ...
  const handleShare = async () => {
    // CRITICAL: Do NOT close the menu immediately. 
    // Removing the element from DOM might cause the browser to lose "User Activation" context 
    // needed for navigator.share, resulting in "Permission denied".
    
    const success = await shareAudio(audioBlob);
    
    setShowShareMenu(false); // Close menu after attempt
    ...
    <div className="flex items-center justify-between w-full h-full gap-3 relative">
       ...
       <div className="flex items-center gap-1 relative">
          <button 
            ...
            className={`p-2 transition-colors rounded-full ${showShareMenu ? 'bg-slate-100 dark:bg-slate-800 text-emerald-500' : 'text-slate-500 hover:text-emerald-500 dark:text-slate-400'}`}
          >
          ...
          <button onClick={onReset} className="p-2 text-slate-400 hover:text-red-500 dark:text-slate-500 dark:hover:text-red-400 transition-colors">

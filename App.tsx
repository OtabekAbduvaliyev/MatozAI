...
import { Copy, Sun, Moon, History, Plus, UploadCloud, Sparkles, Languages, FileDown, FileText } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      geminiService.current.connect(stream, {
        onOpen: () => { console.log('Gemini Ulandi'); },
        onTranscriptionUpdate: handleTranscriptionUpdate,
        onError: (err) => { console.error("Gemini Xatosi", err); },
        onClose: () => { console.log('Gemini Yopildi'); }
      });
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    geminiService.current.transcribeAudioFile(file)
      .then((text) => {
         setFullText(text);
  ...
  const handleTranslate = async (lang: 'en' | 'ru') => {
    ...
    try {
        const translation = await geminiService.current.translateText(fullText, lang);
  ...
  const handleNewConversation = () => {
    ...
    if (fullText || audioBlob) {
        executeReset(true).then(() => setToastMessage("Yangi suhbat boshlandi (Avtomatik saqlandi)"));
    } else {
        executeReset(false);
    }
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.code === 'Space' || e.code === 'Enter') && appState !== 'PROCESSING' && !isConfirmOpen && !isHistoryOpen && !isSummaryOpen && !isTranslationOpen) {
  ...
  }, [appState, isConfirmOpen, isHistoryOpen, isSummaryOpen, isTranslationOpen]);
    ...
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans overflow-hidden transition-colors duration-300 relative">
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
            ...
            executeReset(false).then(() => setToastMessage("Joriy natijalar o'chirildi"));
        }}
